...
\begin{frame}{What is an LLM?}
    \begin{columns}[T]
        \begin{column}{0.42\textwidth}
            \begin{itemize}
                \vspace{0.3em}
                \item LLMs are a type of \textbf{neural network}.
                \vspace{0.3em}
                \item Neural networks are made of \textbf{layers}.
                \vspace{0.3em}
                \item Learning from raw data (pixels) is very hard.
                \vspace{0.3em}
                \item Each layer models increasingly \textbf{abstract features}: edges, contours, object parts, identity.
                \vspace{0.3em}
...
\begin{frame}{But What Did It Actually Learn?}
    \begin{columns}[T]
        \begin{column}{0.62\textwidth}
            \begin{itemize}
                \vspace{0.3em}
                \item These models learn \textbf{statistical patterns}, not \textbf{concepts}.
                \vspace{0.3em}
                \item More layers means more complex features: from edges to shapes to objects, from words to phrases to meaning.
                \vspace{0.3em}
                \item Adding layers changes how the network \textbf{thinks}, not what it knows.
                \vspace{0.3em}
                \item Today: \textbf{fine-tuning} (changing how it thinks). Not RAG (changing what it knows).
                \vspace{0.3em}
                \item This is why LLMs \textbf{hallucinate}: they produce statistically plausible text, not verified facts.
            \end{itemize}
        \end{column}
        \begin{column}{0.35\textwidth}
            \textbf{Ribeiro et al.\ (2016):}\\
            A husky/wolf classifier was detecting \textbf{snow in the background}, not the animal.
...
\begin{frame}{From Images to Language}
    A \textbf{Large Language Model (LLM)} predicts the next word based on patterns learned from massive text data. Not a database, not a search engine: a \textbf{pattern completer}.
...
\begin{frame}[fragile]{It's Just a File}
    ...
    \vspace{0.8em}
    \textbf{Ollama} is a free, open-source app that downloads and runs LLMs on your machine.
...
\begin{frame}{Problem: It Forgot My Introduction}
    ...
    \vspace{0.8em}
    But what if you want the model to reference something you wrote earlier in your paper? It can't. The model can only see $\sim$\textbf{3 pages} of text (4,096 tokens by default).
...
\begin{frame}{Quantization: Making Models Fit}
    ...
    \vspace{0.3em}
    This is why a 3B model fits in 2 GB instead of 12 GB. Models on Ollama's library come pre-quantized (usually Q4). No action needed.
\end{frame}
    ...
    \begin{columns}[T]
        \begin{column}{0.48\textwidth}
            ...
            \begin{tcolorbox}[colback=red!5, colframe=red!40, boxrule=0.5pt, arc=2pt]
                Update \textbf{all} 3 billion parameters
        ...
        \begin{column}{0.48\textwidth}
            ...
            \begin{tcolorbox}[colback=green!5, colframe=green!40, boxrule=0.5pt, arc=2pt]
                Freeze original, add \textbf{small} trainable layers
...
\begin{frame}{Accessing NRP}
    ...
    \textbf{The National Research Platform (NRP)} is a free, NSF-funded GPU compute platform shared across US universities, including Berkeley.
    ...
    \begin{tcolorbox}[colback=red!5, colframe=red!40, boxrule=0.5pt, arc=2pt, fontupper=\small]
        \textbf{Warning --- Pod Reclamation:} NRP is a shared cluster. Your JupyterHub pod can be \textbf{killed without warning} if a higher-priority job needs the GPU. Our training took 3 attempts before finishing!\\[0.3em]
...
\begin{frame}{Training a LoRA Adapter}
    ...
    \vspace{0.5em}
    \textbf{Load} a pretrained model in 4-bit $\rightarrow$ \textbf{attach} a small LoRA adapter $\rightarrow$ \textbf{train} on your data $\rightarrow$ \textbf{save} the adapter ($\sim$100 MB).
...
\begin{frame}[fragile]{From NRP to the World}
    ...
    \vspace{0.6em}
    \textbf{\href{https://huggingface.co}{Hugging Face}} = GitHub for AI models. Upload the adapter ($\sim$130 MB) once, anyone can pull it down.
...
\begin{frame}{Deployment Options}
    ...
    \vspace{0.5em}
    \textbf{Key:} Berkeley provides \textbf{Colab Pro} for free --- gives you T4/V100 GPUs and longer runtimes.
...
\begin{frame}{When to Fine-Tune vs.\ When to Just Prompt}
    ...
    \begin{table}
        ...
        \begin{tabular}{lll}
            ...
            Example & ``Rewrite this more formally'' & Training on 2,000 abstracts \\
